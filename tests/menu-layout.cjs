...
(async () => {
  ...
  try {
    ...
    assert.equal(await panel.getByRole('tab', { name: 'Ads', exact: true }).getAttribute('aria-selected'), 'true');
    await panel.getByRole('tab', { name: 'Ads', exact: true }).press('Home');
    assert.equal(await panel.getByRole('tab', { name: 'Promotions', exact: true }).getAttribute('aria-selected'), 'true');
    ...
    assert.equal(await panel.getByRole('button', { name: 'Export settings', exact: true }).count(), 1);
    assert.equal(await panel.getByRole('button', { name: 'Import settings', exact: true }).count(), 1);
    assert.equal(await panel.getByRole('button', { name: 'Scan selector coverage', exact: true }).count(), 1);
    await panel.getByRole('button', { name: 'About & diagnostics', exact: true }).click();
    await settle();
    ...
    for (const viewport of [
      ...
      if (viewport.height < 560) {
        ...
        const headings = await panel.getByRole('tab').all();
    ...
    await fab.click();
    await panel.getByRole('tab', { name: /^Promotions/ }).click();
    await settle();
